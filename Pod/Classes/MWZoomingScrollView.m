...
- (id)initWithPhotoBrowser:(MWPhotoBrowser *)browser {
    if ((self = [super init])) {
        ...
        if (browser.progressColor != nil) {
            _loadingIndicator.progressTintColor = browser.progressColor;
        }
        if (browser.trackTintColor != nil) {
            _loadingIndicator.trackTintColor = browser.trackTintColor;
        }
...
- (void)displayImageFailure {
    ...
    if (![_photo respondsToSelector:@selector(emptyImage)] || !_photo.emptyImage) {
        ...
        _loadingError.frame = CGRectMake(floorf((float)((self.bounds.size.width - _loadingError.frame.size.width) / 2.)),
...
- (void)setMaxMinZoomScalesForCurrentBounds {
    ...
    if (self.zoomScale != minScale) {
        ...
        // Centralise
        self.contentOffset = CGPointMake((CGFloat)((imageSize.width * self.zoomScale - boundsSize.width) / 2.0),
                (CGFloat)((imageSize.height * self.zoomScale - boundsSize.height) / 2.0));
...
- (void)layoutSubviews {
    ...
    if (!_loadingIndicator.hidden)
        _loadingIndicator.frame = CGRectMake(floorf((float)((self.bounds.size.width - _loadingIndicator.frame.size.width) / 2.)),
    ...
    if (frameToCenter.size.width < boundsSize.width) {
        frameToCenter.origin.x = floorf((float)((boundsSize.width - frameToCenter.size.width) / 2.0));
    } else {
    ...
    if (frameToCenter.size.height < boundsSize.height) {
        frameToCenter.origin.y = floorf((float)((boundsSize.height - frameToCenter.size.height) / 2.0));
    } else {
